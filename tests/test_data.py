...
# ---------------------------------------------------------------------------
# Tests for cross-season baseline exposure (XSBL-01, XSBL-02, XSBL-03)
# ---------------------------------------------------------------------------
...
def _create_synthetic_multi_year_data(tmp_path, years=(2025, 2026)):
    ...
    # Create parquet files for each year
    for year in years:
        ...
        statcast_df = pl.DataFrame(
            {
                ...
                "p_throws": ["R", "R", "R"],
                "game_type": ["R", "R", "R"],
                "game_year": [year, year, year],
                "game_pk": [100000 + year, 100000 + year, 100000 + year],
                "game_date": [game_dt, game_dt, game_dt],
                "inning": [1, 1, 2],
            }
        )
        statcast_df.write_parquet(tmp_path / f"statcast_{year}.parquet")

    # Create CSV agg files for each year and grain
    for year in years:
        for grain in [*_SEASON_GRAINS, *_APPEARANCE_GRAINS]:
            base_cols: dict[str, list] = {
                ...
                "n_pitches": [100],
                "stuff_plus": [100.0 + (year - 2025) * 10],
                "location_plus": [95.0 + (year - 2025) * 5],
                "pitching_plus": [98.0 + (year - 2025) * 7],
            }
            if grain != "team":
                base_cols["pitcher"] = [12345]
            if "type" in grain:
...
def test_xsbl01_multi_season_baselines_populated(tmp_path, monkeypatch):
    """XSBL-01: Multi-season pitcher gets both current and prior baselines populated."""
    aggs_dir = _create_synthetic_multi_year_data(tmp_path, years=(2025, 2026))
    ...
    data = load_pitcher_data(12345, window_days=365)

    # Current season baseline should be 2026 only
    assert not data.season_baseline.is_empty()
    assert data.season_baseline["season"].unique().to_list() == [2026]

    # Prior season baseline should be 2025
    assert not data.prior_season_baseline.is_empty()
    assert data.prior_season_baseline["season"].unique().to_list() == [2025]

    # Current pitch type baseline should be 2026 only
    assert not data.pitch_type_baseline.is_empty()
    assert data.pitch_type_baseline["season"].unique().to_list() == [2026]

    # Prior pitch type baseline should be 2025
    assert not data.prior_pitch_type_baseline.is_empty()
    assert data.prior_pitch_type_baseline["season"].unique().to_list() == [2025]


def test_xsbl01_prior_baseline_has_correct_metrics(tmp_path, monkeypatch):
    """XSBL-01: Prior-season baselines contain the correct metric values."""
    aggs_dir = _create_synthetic_multi_year_data(tmp_path, years=(2025, 2026))
    ...
    # 2025 stuff_plus = 100.0, 2026 = 110.0
    assert abs(data.season_baseline["stuff_plus"][0] - 110.0) < 0.01
    assert abs(data.prior_season_baseline["stuff_plus"][0] - 100.0) < 0.01


def test_xsbl02_load_pitcher_data_retains_all_seasons(tmp_path, monkeypatch):
    """XSBL-02: load_pitcher_data retains all per-season data (current + prior)."""
    aggs_dir = _create_synthetic_multi_year_data(tmp_path, years=(2025, 2026))
    ...
    # Combined current + prior should have both seasons
    combined_seasons = set(data.season_baseline["season"].to_list()) | set(
        data.prior_season_baseline["season"].to_list()
    )
    assert combined_seasons == {2025, 2026}

    combined_pt_seasons = set(data.pitch_type_baseline["season"].to_list()) | set(
        data.prior_pitch_type_baseline["season"].to_list()
    )
    assert combined_pt_seasons == {2025, 2026}


def test_xsbl03_single_season_prior_empty(tmp_path, monkeypatch):
    """XSBL-03: Single-season pitcher gets empty (not None, not crash) prior baselines."""
    aggs_dir = _create_synthetic_multi_year_data(tmp_path, years=(2026,))

    import pitcher_narratives.data as data_mod

    monkeypatch.setattr(data_mod, "DATA_DIR", tmp_path)
    monkeypatch.setattr(data_mod, "AGGS_DIR", aggs_dir)
    monkeypatch.setattr(data_mod, "_YEARS", [2026])

    data = load_pitcher_data(12345, window_days=365)

    # Current baselines should be populated
    assert not data.season_baseline.is_empty()
    assert not data.pitch_type_baseline.is_empty()

    # Prior baselines should be empty DataFrames (not None)
...
def test_xsbl03_prior_empty_preserves_schema(tmp_path, monkeypatch):
    """XSBL-03: Empty prior baselines have the same schema as current baselines."""
    aggs_dir = _create_synthetic_multi_year_data(tmp_path, years=(2026,))
    ...
    # Empty prior baselines should have same columns as current baselines
    assert set(data.prior_season_baseline.columns) == set(data.season_baseline.columns)
    assert set(data.prior_pitch_type_baseline.columns) == set(
        data.pitch_type_baseline.columns
    )


def test_xsbl_regression_current_baseline_unchanged(tmp_path, monkeypatch):
    """Regression: Current-season baselines still contain max season only."""
    aggs_dir = _create_synthetic_multi_year_data(tmp_path, years=(2025, 2026))
    ...
    # season_baseline should ONLY have 2026 (max season) -- same as before
    assert len(data.season_baseline) == 1
    assert data.season_baseline["season"][0] == 2026

    # pitch_type_baseline should ONLY have 2026 -- same as before
    assert all(s == 2026 for s in data.pitch_type_baseline["season"].to_list())


def test_xsbl_pitcher_data_has_prior_fields():
    """XSBL-01: PitcherData dataclass has prior baseline fields."""
    import dataclasses

    from pitcher_narratives.data import PitcherData

    field_names = [f.name for f in dataclasses.fields(PitcherData)]
    assert "prior_season_baseline" in field_names
    assert "prior_pitch_type_baseline" in field_names


def test_xsbl_multi_pitch_type_prior(tmp_path, monkeypatch):
    """XSBL-01: Prior pitch-type baselines work with multiple pitch types."""
    from datetime import date as date_cls
    ...
    # Create CSV agg files with multiple pitch types
    for year in [2025, 2026]:
        for grain in [*_SEASON_GRAINS, *_APPEARANCE_GRAINS]:
            if "type" in grain and grain != "team":
                # Two pitch types: FF and SL
                rows: dict[str, list] = {
                    "season": [year, year],
                    "game_type": ["R", "R"],
                    ...
                    "team_code": ["NYY", "NYY"],
                    "n_pitches": [70, 30],
                    "stuff_plus": [105.0 + (year - 2025) * 10, 95.0 + (year - 2025) * 5],
                    "pitcher": [12345, 12345],
                    "pitch_type": ["FF", "SL"],
                }
                if "platoon" in grain:
                    rows["platoon"] = ["vs_R", "vs_R"]
                if "appearance" in grain:
                    rows["game_date"] = [f"{year}-06-15", f"{year}-06-15"]
                    rows["game_pk"] = [100000 + year, 100000 + year]
            else:
                rows = {
                    "season": [year],
                    "game_type": ["R"],
                ...
                if grain != "team":
                    rows["pitcher"] = [12345]
                if grain == "all_pitches":
                    rows["game_date"] = [f"{year}-06-15"]
                    rows["game_pk"] = [100000 + year]
    ...
    # Prior pitch_type_baseline should have both FF and SL for 2025
    assert len(data.prior_pitch_type_baseline) == 2
    prior_types = set(data.prior_pitch_type_baseline["pitch_type"].to_list())
    assert prior_types == {"FF", "SL"}
    assert all(s == 2025 for s in data.prior_pitch_type_baseline["season"].to_list())

    # Current should have 2026
    assert len(data.pitch_type_baseline) == 2
    assert all(s == 2026 for s in data.pitch_type_baseline["season"].to_list())
