...
_RESIDUAL_THRESHOLD_IN = 2.0
"""Inches of residual below which movement matches slot expectation."""
...
def _classify_shape(run_residual_in: float, ride_residual_in: float, is_fastball: bool) -> str:
    """Classify a pitch's movement residuals vs slot expectation.

    Fastballs whose movement matches slot expectation on both axes are
    tagged DEAD ZONE (hitters see exactly what the slot predicts).
    Residuals of at least 2 inches on either axis are flagged as
    distinctive traits.
    """
    flags: list[str] = []
    if ride_residual_in >= _RESIDUAL_THRESHOLD_IN:
        flags.append(f"ride above slot expectation ({ride_residual_in:+.1f} in)")
    elif ride_residual_in <= -_RESIDUAL_THRESHOLD_IN:
        flags.append(f"sinks below slot expectation ({ride_residual_in:+.1f} in)")
    if run_residual_in >= _RESIDUAL_THRESHOLD_IN:
        flags.append(f"more arm-side run than slot suggests ({run_residual_in:+.1f} in)")
    elif run_residual_in <= -_RESIDUAL_THRESHOLD_IN:
        flags.append(f"more cut/glove-side than slot suggests ({run_residual_in:+.1f} in)")
...
def _interpolate_expectation(
    ...
    """Linearly interpolate expected (run, ride) at an exact arm angle.

    Bucket means are anchored at bucket centers (bucket + 5 deg) and the
    ...
    edges. When one neighbor is missing (edge of the league table) the
    nearest available bucket's means are used unchanged.

    Returns:
        (exp_arm_side_run_in, exp_ride_in) or None when no bucket near
        the angle exists for this pitch type.
    """
    ...
    if lower is not None and upper is not None:
        ...
        run = lower.exp_arm_side_run_in + t * (upper.exp_arm_side_run_in - lower.exp_arm_side_run_in)
        ride = lower.exp_ride_in + t * (upper.exp_ride_in - lower.exp_ride_in)
        return run, ride

    nearest = lower if lower is not None else upper
    if nearest is None:
        return None
    return nearest.exp_arm_side_run_in, nearest.exp_ride_in
...
def compute_slot_expectations() -> dict[tuple[str, int], SlotExpectation]:
    ...
    df = load_all_statcast(columns=["pitch_type", "p_throws", "arm_angle", "pfx_x", "pfx_z"])
    df = df.filter(
    ...
    agg = (
        ...
        .agg(
            ...
            pl.col("arm_side_run_in").std().alias("std_run"),
            pl.col("ride_in").mean().alias("exp_ride"),
            pl.col("ride_in").std().alias("std_ride"),
        )
    ...
    for row in agg.iter_rows(named=True):
        ...
        table[key] = SlotExpectation(
            ...
            std_arm_side_run_in=float(row["std_run"]) if row["std_run"] is not None else 0.0,
            std_ride_in=float(row["std_ride"]) if row["std_ride"] is not None else 0.0,
        )
    ...
    for row in agg.iter_rows(named=True):
        ...
        entries.append(
            PitchShapeEntry(
                ...
                shape_tag=_classify_shape(run_residual, ride_residual, is_fastball),
            )
        )
...
def render_pitch_shape(profile: PitchShapeProfile | None) -> str:
    ...
    lines.append(
        ...
        "A fastball whose ride and run both sit within 2 in of slot "
        "expectation is a DEAD ZONE pitch -- the exact shape hitters' "
        "eyes predict from the release slot. Large residuals are "
        "deceptive traits."
    )
