...
5. **Writer** (`personas.py` `SHARED_WRITER_BASE`): one bullet telling the writer to keep the insight during synthesis. Then **regenerate** `tests/fixtures/writer_prompt_*.txt` via `build_writer_system_prompt` — the byte-identical fixture tests exist to catch *accidental* drift; intentional prompt changes regenerate them.
